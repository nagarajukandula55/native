...
export default function SanthaOrdersPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push("/login?next=/santha/orders");
      return;
    }
    let cancelled = false;
    setLoading(true);
  ...
  return (
    <div className="container">
      <div className="headRow">
        <h1>My Santha Orders</h1>
        <Link href="/santha" className="link">
          + New Order
        </Link>
      </div>
      ...
      {!loading && !error && !orders.length && (
        <div className="empty">
          <p>No santha orders yet.</p>
          <Link href="/santha" className="shopLink">
      ...
      <div className="list">
        {orders.map((order) => {
          ...
          return (
            <Link key={order._id} href={`/santha/orders/${order._id}`} className="row">
              <div className="rowMain">
                <p className="orderId">Order #{String(order._id).slice(-6).toUpperCase()}</p>
                <p className="meta">
                  {order.plannedFor
                    ? `Planned ${new Date(order.plannedFor).toLocaleDateString()} · `
                    : ""}
              ...
              <div className="rowRight">
                ...
                {order.totalAmount != null && <span className="total">₹{order.totalAmount}</span>}
              </div>
            </Link>
          );
        })}
