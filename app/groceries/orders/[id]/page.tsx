...
export default function GroceryOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push(`/login?next=/groceries/orders/${id}`);
      return;
    }
    if (!id) return;
    let cancelled = false;
  ...
  return (
    <div className="container">
      <Link href="/groceries/orders" className="back">
        ← My Grocery Orders
      </Link>
      ...
      <div className="section">
        <h2>Quote</h2>
        {!hasQuote && <p className="muted">Awaiting quote from the shop.</p>}
        {hasQuote && (
          <>
