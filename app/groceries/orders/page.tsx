...
export default function MonthlyGroceryOrdersPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push("/login?next=/groceries/orders");
      return;
    }
    let cancelled = false;
    setLoading(true);
  ...
  return (
    <div className="container">
      <div className="headRow">
        ...
        <Link href="/groceries" className="link">
          + New Order
        </Link>
      </div>
      ...
      {!loading && !error && !orders.length && (
        <div className="empty">
          <p>No monthly grocery orders yet.</p>
          <Link href="/groceries" className="shopLink">
      ...
      <div className="list">
        {orders.map((order) => {
          ...
          return (
            <Link key={order._id} href={`/groceries/orders/${order._id}`} className="row">
