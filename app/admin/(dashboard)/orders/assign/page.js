...
export default function AssignOrders() {
  ...
  const [selected, setSelected] = useState({}); // store + warehouse per order
  ...
  async function handleAssign(orderId) {
    ...
    if (!sel?.storeId || !sel?.warehouseId) return alert("Select store & warehouse");
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "auto", padding: 20 }}>
      <h1 style={{ fontSize: 28, fontWeight: "bold" }}>📝 Assign Orders to Store & Warehouse</h1>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <select
              ...
              onChange={(e) =>
                setSelected({
                  ...selected,
                  [order._id]: { ...selected[order._id], storeId: e.target.value },
                })
              }
              disabled={assigning}
            >
            ...
            <select
              ...
              onChange={(e) =>
                setSelected({
                  ...selected,
                  [order._id]: { ...selected[order._id], warehouseId: e.target.value },
                })
              }
              disabled={assigning}
            >
              ...
              {stores.map(
                (s) =>
                  s.warehouseId && selected[order._id]?.storeId === s._id && (
